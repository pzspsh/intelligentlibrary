...
**3. 多轮对话中的生成。**在使用 ChatGPT 等应用时，用户往往会根据模型的输出反复提出修改要求。在这种多轮对话的场景下，模型的多次输出往往只有少量的变化，重复度较高。![image-20240125141655477](../../images/image-20240125141655477.png)
...
![image-20240125141748885](../../images/image-20240125141748885.png)
...
![image-20240125141939831](../../images/image-20240125141939831.png)

论文链接：[https://arxiv.org/pdf/2304.04487.pdf](https://weibo.cn/sinaurl?u=https%3A%2F%2Farxiv.org%2Fpdf%2F2304.04487.pdf)

项目链接：[https://github.com/microsoft/LMOps](https://weibo.cn/sinaurl?u=https%3A%2F%2Fgithub.com%2Fmicrosoft%2FLMOps)
...
![image-20240125142001557](../../images/image-20240125142001557.png)
表 1：检索增强的生成场景下的时间对比

![image-20240125142021397](../../images/image-20240125142021397.png)
表 2：使用缓存的生成场景下的时间对比
...
![image-20240125142101959](../../images/image-20240125142101959.png)
...
进一步实验分析发现，LLM Accelertator 能显著减少所需的解码步骤，并且加速比与解码步骤的减少比例呈正相关。更少的解码步骤一方面意味着每个解码步骤生成的输出词数更多，可以提高 GPU 计算的计算效率；另一方面，对于需要多卡并行的 30B 模型，这意味着更少的多卡同步，从而达到更快的速度提升。在消融实验中，在开发集上对 LLM Accelertator 的超参数进行分析的结果显示，匹配单个单词（即触发拷贝机制）时，一次拷贝 15 到 20 个单词时的加速比可达到最大 （图 4 所示）。在图 5 中我们可以看出，匹配词数为 1 能更多地触发拷贝机制，并且随着拷贝长度的增加，每个解码步骤接受的输出词增加，解码步骤减少，从而达到更高的加速比。

![image-20240125142122577](../../images/image-20240125142122577.png)
图 4：消融实验中，在开发集上对 LLM Accelertator 的超参数的分析结果

![image-20240125142139428](../../images/image-20240125142139428.png)
