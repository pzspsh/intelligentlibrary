...
class ParticleSwarmOpt:
    ...
    def __init__(
        ...
        if isinstance(self.best_unit, np.ndarray):
            self.particle[0] = self.best_unit
    ...
    def generate(self, n: int) -> np.ndarray:
        ...
        return (
            x * (self.coord_info[:, 1] - self.coord_info[:, 0]) + self.coord_info[:, 0]
        )
    ...
    def fit(
        ...
        for i in pbar:
            ...
            if need:
                self.particle = np.concatenate(
                    [self.particle, self.generate(need)], axis=0
                )
                self.inertia = np.concatenate(
                    [self.inertia, np.zeros([need, len(self.coord_info)])]
                )
            # 展示进度
            pbar.set_description((f"%-10s" + "%-10.4g") % (prefix, self.best_fitness))
        pbar.close()
        return self.best_unit, pd.DataFrame(
            self.log, columns=["fit-best", "fit-mean", "fit-std", "n-unique"]
        )
    ...
    def _move_pace(self) -> np.ndarray:
        ...
        # 粒子间的距离
        direct = (
            np.append(self.particle, self.best_unit[None], axis=0)
            - self.particle[:, None]
        )
