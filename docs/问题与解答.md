...
```go
    ...
    2、func Logger(pathfile string, args ...any, namelist ...string, nameint ...int) {
    }
...
##### 67、在Linux中将可执行文件设为全局可用问题
```bash
方法一：
# 将可执行文件复制到/usr/local/bin/目录：
    sudo cp <binary-name> /usr/local/bin/
例：
    cp /path/<binary-name> /usr/local/bin/<binary-name>

# 或生成链接：
    sudo ln -s /absolute/path/to/binary /usr/local/bin/<binary-name>

方法二：
echo "export PATH=$PATH:/the/path/to/software" >> ~/.profile && source ~/.profile

/usr/local/bin/用户安装的
/usr/bin/系统安装的，系统更新时可能发生覆盖
```

##### 68、ERROR: Cannot uninstall urllib3 2.0.7, RECORD file not found.  Hint: The package was installed by debian.
```bash
解决办法：
pip install --ignore-installed urllib3 -i https://mirrors.aliyun.com/pypi/simple  --break-system-packages
```

##### 69、Golang 报at least one file in a package should have a package commentST1000default警告
```bash
解决办法：安装gopls的老版本
# 查看gopls版本  https://pkg.go.dev/golang.org/x/tools/gopls?tab=versions
...
或修改配置文件：
{
    ...
    "staticcheck": false,  // use staticcheck for type checking  // 修改staticcheck为false, 禁用静态检查
    "analyses": {
    "unusedparams": true,
    "unusedwrite": true}
...
##### 71、报“加载工作区时出错：packages.Load error: err: exit status 1: stderr: go: no modules were found in the current workspace; see 'go help work'””
```
如果文件夹下有多个Go项目：
go work init
go work use
...
##### 72、报ERROR: Cannot uninstall rich 13.7.1, RECORD file not found. 。。。错误
```bash
pip uninstall rich # 错误方法
sudo apt-get remove python3-rich # 正确方法
```