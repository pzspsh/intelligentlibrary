...
import (
	...
	"function/smb/smb/gss"
	"function/smb/smb/ntlmssp"
...
func NewSession(opt Options, debug bool) (s *Session, err error) {
	...
	if opt.Proxy != "" {
		dialer, _ := proxy.SOCKS5("tcp", opt.Proxy, nil, proxy.Direct)
		conn, err = dialer.Dial("tcp", fmt.Sprintf("%v:%v", opt.Host, opt.Port))
		if err != nil {
			return nil, err
		}
	} else {
