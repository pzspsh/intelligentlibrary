...
func Dial(addr, proxy1 string, options ...DialOption) (*ServerConn, error) {
	do := &dialOptions{}
	...
	if dialFunc == nil {
		...
		if do.tlsConfig != nil && !do.explicitTLS {
			dialFunc = func(network, address string) (net.Conn, error) {
				...
				if proxy1 != "" {
					dialer, _ := proxy.SOCKS5("tcp", proxy1, nil, proxy.Direct)
...
func Connect(addr, proxy string) (*ServerConn, error) {
	return Dial(addr, proxy)
}

// DialTimeout initializes the connection to the specified ftp server address.
//
// Deprecated: Use [Dial] with [DialWithTimeout] option instead
func DialTimeout(addr, proxy string, timeout time.Duration) (*ServerConn, error) {
	return Dial(addr, proxy, DialWithTimeout(timeout))
}
