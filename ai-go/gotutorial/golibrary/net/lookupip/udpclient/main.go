...
*
 */
func NewService(name, addr string, port uint32, timeout time.Duration) *Service {
...
func (s *Service) Request(message int64) ([]int64, error) {
	...
	if err != nil {
		fmt.Printf("udp request data pack err:%s", err)
		return nil, err
	}
	conn.SetDeadline(time.Now().Add(s.Timeout * time.Second))
	if _, err := conn.Write(data); err != nil {
		fmt.Printf("udp write to serve err:%s", err)
		return nil, err
	}
	buf := make([]byte, 1<<10)
	if _, err := conn.Read(buf); err != nil {
		fmt.Printf("udp read from serve err:%s", err)
		return nil, err
	}
	msg, err := dp.UnPack(buf)
	if err != nil {
		fmt.Printf("udp response data unpack err:%s", err)
		return nil, err
	}
	return msg, nil
}
...
func NewDataPack() *Protocol {
	pro := &Protocol{}
	return pro
}

// 编码
func (p *Protocol) Pack(args ...int64) ([]byte, error) {
	la := len(args)
	ls := len(p.Format)
	ret := make([]byte, 8)
	if ls > 0 && la > 0 && ls == la {
...
// 解码
func (p *Protocol) UnPack(data []byte) ([]int64, error) {
	la := len(p.Format)
	ret := make([]int64, la)
	if la > 0 {
		for i := 0; i < la; i++ {
