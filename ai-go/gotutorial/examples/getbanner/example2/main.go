...
func main() {
	// 获取HTTP服务的Banner信息
	httpBanner := getHTTPBanner("http://www.example.com:80")
	fmt.Println("HTTP Banner:", httpBanner)

	// 获取HTTPS服务的Banner信息
	httpsBanner := getHTTPSBanner("https://www.example.com:443")
	fmt.Println("HTTPS Banner:", httpsBanner)

	// 获取FTP服务的Banner信息
	ftpBanner := getFTPBanner("ftp://ftp.example.com:21")
	fmt.Println("FTP Banner:", ftpBanner)

	// 获取SSH服务的Banner信息
	sshBanner := getSSHBanner("ssh://example.com:22")
	fmt.Println("SSH Banner:", sshBanner)

	// 获取MySQL服务的Banner信息
	mysqlBanner := getMySQLBanner("example.com:3306")
...
func getSSHBanner(url string) string {
	...
	return "SSH Banner: " + conn.RemoteAddr().String()
}
