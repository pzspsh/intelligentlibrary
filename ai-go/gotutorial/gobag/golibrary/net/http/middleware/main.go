...
func main() {
	...
	//中间件可充当业务逻辑代码
	sliceRouter.Group("/").Use(Url(), TraceLogSliceMW(), func(c *SliceRouterContext) {
		fmt.Println("中间件")
	})
	sliceRouter.Group("/topgoer").Use(Url(), TraceLogSliceMW(), topgoer)
...
func topgoer(c *SliceRouterContext) {
